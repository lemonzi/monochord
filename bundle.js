...
$.extend(MonoChord.prototype, {
    play: function(when, where) {
        ...
        noise.setValueAtTime(.3, t);
        lop.setValueAtTime(8e3, t);
        t += .03;
        noise.exponentialRampToValueAtTime(.001, t);
        t += 2;
        lop.exponentialRampToValueAtTime(1e3, t);
        noise.linearRampToValueAtTime(0, t);
        t += 1.2;
        env.exponentialRampToValueAtTime(.4, t);
        t += 3;
        env.exponentialRampToValueAtTime(.01, t);
        lop.exponentialRampToValueAtTime(100, t);
        env.setTargetAtTime(0, t, .02);
        lop.setTargetAtTime(0, t, .02);
        this.timer = setTimeout(function() {
            this.nodes.osc.stop();
            this.nodes.osc.disconnect();
            this.nodes.osc = null;
            this.timer = null;
            this.nodes.losses.frequency.value = 2e4;
    ...
    setBeta: function(beta) {
        ...
        this.nodes.osc.setPeriodicWave(this.ctx.createPeriodicWave(real, imag))
    }
});
function MonoChordUI(opts) {
    ...
    this.knobs.chord.data({
        min: this.range.min,
        max: this.range.max
    });
    this.knobs.fine.data({
        incr: .05,
        min: 1,
        max: 1e3
    });
    this.knobs.superfine.data({
        incr: .005,
        min: 1,
        max: 1e3
    });
...
$(function() {
    ...
    var noise_worklet = URL.createObjectURL( new Blob([ '(', function(){
            ...
            process(inputs, outputs) { 
                ...
                for (let channel = 0; channel < output.length; ++channel) {
                    ...
                    for (let i = 0; i < outputChannel.length; ++i) {
                        outputChannel[i] = 2 * (Math.random() - 0.5) * 2
                    }
                }
                return true; 
            }
        }
        registerProcessor('noise_worklet', NoiseWorkletProcessor);
    }.toString(), ')()' ], { type: 'application/javascript' }));
    ctx.audioWorklet.addModule(noise_worklet).then(function() {
        ...
        var jitter = getQueryVariable("jitter", "number") || .03;
        function playAll(e) {
            ctx.resume();
        ...
        var where = getQueryVariable("timbre", "number") || .01;
        var monochords = [];
